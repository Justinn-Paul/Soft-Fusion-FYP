...
class SDFusionText2ShapeModel(BaseModel):
    ...
    def init_diffusion_params(self, uc_scale=3., opt=None):
        ...
        logvar_init = 0.
        self.logvar = torch.full(fill_value=logvar_init, size=(self.num_timesteps,)).to(self.device) # penway add to device
    ...
    def load_ckpt(self, ckpt, load_opt=False):
        ...
        # penway, optimizer ckpt not offered in official pretrained model, so this is commented
        # if load_opt:

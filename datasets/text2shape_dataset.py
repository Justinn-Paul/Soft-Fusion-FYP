...
class Text2ShapeDataset(BaseDataset):
    ...
    def initialize(self, opt, phase='train', cat='all', res=64):
        ...
        self.text_csv = f'{dataroot}/ShapeNet/text2shape/captions.robot_{phase}.csv'
        ...
        assert cat.lower() in ['all', 'bending', 'robot']  # penway ['all', 'chair', 'table']
        if cat == 'all':
            valid_cats = ['bending', 'robot']
        else:
            valid_cats = [cat]
        ...
        for d in tqdm(self.data, total=len(self.data), desc=f'readinging text data from {self.text_csv}'):
            ...
            if cat_i.lower() not in valid_cats:
                cprint(f"######### {cat_i} not in {valid_cats}", 'red')
                continue
            ...
            if not os.path.exists(sdf_path):
                cprint(f"######### {sdf_path} not found", 'red')
                continue

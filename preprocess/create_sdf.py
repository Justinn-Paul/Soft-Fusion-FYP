...
parser.add_argument('--dset', type=str, choices=['shapenet', 'abc', 'pix3d', 'building', 'softnet'], default='shapenet', help='which dataset to extract sdf')
...
def create_sdf_softnet(sdfcommand, marching_cube_command, LIB_command,
               num_sample, bandwidth, res, expand_rate,
               lst_dir, all_cats, raw_dirs, iso_val,
               max_verts, ish5=True, normalize=True, g=0.00, reduce=4):
    '''
...
if __name__ == "__main__":
    ...
    info_file = '../data/ShapeNet/info.json'
    # info_file = "/root/autodl-tmp/SDFusion/dataset_info_files/info-shapenet.json"
    ...
    if dset == 'shapenet' or dset == 'softnet':
        if cat != 'all':
            cats = {cat: cats[cat]}
    elif dset != 'abc':
        if cat == 'all':
    ...
    isosurface_dir = './isosurface'  # penway './isosurface/'
    ...
    if dset == 'abc':
        ...
        create_sdf_softnet(sdf_cmd, mcube_cmd, "source %s" % lib_cmd,
                   num_sample, bandwidth, res, expand_rate,
                   lst_dir, cats, raw_dirs,
                   iso_val, max_verts, ish5=True, normalize=True, g=0.00, reduce=reduce)